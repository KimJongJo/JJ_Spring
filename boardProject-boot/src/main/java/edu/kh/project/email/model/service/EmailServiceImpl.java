...
public class EmailServiceImpl implements EmailService{
	...
	// 타임리프(템플릿 엔진)를 이용해서 html 코드 -> java로 변환
	...
	public String sendEmail(String htmlName, String email) {
		...
		// 1) 해당 이메일이 DB에 존재하는 경우가 있을 수 있기 때문에(이미 인증번호를 받았을 때)
		//    수정(update)을 먼저 진행
		//    -> 1 반환 == 업데이트 성공 == 이미 존재해서 인증번호로 변경(기존에 있던 이메일로 인증번호 재요청)
		//    -> 0 반환 == 업데이트 실패 == 이메일이 존재 X(처음 인증번호 받은 이메일) --> INSERT 시도(DB에 추가)
		
		int result = mapper.updateAuthKey(map);
		
		// 2) 1번 update 실패 시 insert 시도
		if(result == 0) { // 업데이트 실패 (처음 인증번호 받은 이메일)
			
			result = mapper.insertAuthKey(map);
			
		}
		
		// 수정, 삽입 후에도 result 가 0 == 실패 (모두 다 실패)
 ...
 *  
 *  - Java Mail Sender에 Google STMP 이용 설정 추가
