...
public class ImageDeleteScheduling { // 스케줄러를 사용하려면 Bean으로 등록이 되어있어야 한다.
									// 객체화
...
//	@Scheduled(cron = "0,30 * * * * *") // 시계 초 단위가 0, 30 인 경우 수행 (테스트용)
	public void scheduling() {
		...
		// 두 배열을 하나로 합침 (for문 한번만 사용하기 위해서..)
		// imageArr 이라는 빈 배열을 boardArr과 memberArr의 길이 만큼의 크기로 만든다
		File[] imageArr = new File[boardArr.length + memberArr.length];
		
		// 배열 내용 복사 (깊은 복사)
		// System.arraycopy(복사할배열, 몇번인덱스부터 복사할지, 새로운배열, 새로운 배열의 몇번부터 넣을지 인덱스 번호, 복사를 어디까지 할 건지)
